...
<!--                                    <h3 class="card-title">Bordered Table</h3>-->
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="input-group float-right mb-3 col-sm-3">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text"><i class="fa fa-search"></i></span>
                                        </div>
                                        <input class="form-control" type="text" @keyup="search_users()" v-model="search_value">
                                    </div>
                                        ...
                                        <tbody>
                                            <template>
                                                <tr
                                                    ...
                                                    :key="user.id"
                                                >
                                                    <td>{{ ( users.pagination.from + user_index ) }}</td>
                                                    <td>{{ user.name }}</td>
                                                    <td>{{ user.email }}</td>
                                                    ...
                                                    <td>
                                                        <div class="btn-group">
                                                            <router-link :to="{ name: 'UsersEdit', params: { id: user.id } }" class="btn btn-primary">Edit</router-link>
                                    ...
                                    <ul class="pagination pagination-sm m-0 float-right">
                                        ...
                                        >
                                            <a class="page-link" href="#"
                                                @click.prevent="load_users(1)"
                                            >&laquo;</a></li>
                                        <li class="page-item"
                                            :class="users.pagination.current_page == 1 ? 'disabled' : ''"
                                        >
                                            <a class="page-link" href="#"
                                                @click.prevent="load_users(users.pagination.current_page-1)"
                                            >prev</a></li>
                                        <li
                                        ...
                                        >
                                            <a class="page-link" href="#" @click.prevent="load_users(index_page+1)">{{ index_page+1 }}</a>
                                        </li>
                                        <li class="page-item"
                                        ...
                                        >
                                            <a class="page-link" href="#"
                                                @click.prevent="load_users(users.pagination.last_page)"
                                            >&raquo;</a></li>
                                    </ul>
                                </div> -->

                                <!-- This is the pagination template -->
                                <Pagination :change_title="change_title('new title')">
                                </Pagination>
...
export default {
    ...
    methods: {
        render() {
            this.pagination_title = 'new title';
        },

        load_users(page_num) {
            // default page
            const s_value = this.search_value;
            if( s_value != '') {
                axios.get(`/api/users/data?page=${ page_num > 1 ? page_num : 1 }&search_value=${ s_value }`)
                .then(response => {
                ...
                axios.get(`/api/users/data?page=${ page_num > 1 ? page_num : 1 }`)
                .then(response => {
                    this.users = response.data.data;
                    this.users.pagination = response.data;
                   console.log(response.data);
        ...
        search_users() {
            ...
            if( s_value != '' && s_value != null) {
                axios.get(`/api/users/data?search_value=${s_value}`)
                    .then(response => {
                        this.users = response.data.data;
                        this.users.pagination = response.data;
                        console.log(response.data);
                ...
                axios.get(`/api/users/data?page=1`)
                .then(response => {
                    this.users = response.data.data;
                    this.users.pagination = response.data;
                   console.log(response.data);
